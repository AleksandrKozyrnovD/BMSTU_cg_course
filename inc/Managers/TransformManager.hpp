...
{
    Point center = obj->get_center();

    std::shared_ptr<AbstractVisitor> move_to_center = std::make_shared<MoveVisitor>(-center.position.x, -center.position.y, -center.position.z);
    obj->accept(move_to_center);

    std::shared_ptr<AbstractVisitor> visitor = std::make_shared<RotateVisitor>(x, y, z);
    obj->accept(visitor);

    std::shared_ptr<AbstractVisitor> move_back = std::make_shared<MoveVisitor>(center.position.x, center.position.y, center.position.z);
